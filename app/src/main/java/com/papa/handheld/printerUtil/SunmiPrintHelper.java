package com.papa.handheld.printerUtil;
...
public class SunmiPrintHelper {
     ...
     * result)
     */
    public void printTrans(Context context, TicketInfo ticketInfo, InnerResultCallbcak callbcak) {
        if (sunmiPrinterService == null) {
            //TODO Service disconnection processing
            return;
        }
...
//            printExample(context);
            printTicket(context, ticketInfo);
    ...
    private void printTicket(Context context, TicketInfo ticketInfo) {
...
//            sunmiPrinterService.sendRAWData(new byte[]{0x1B,0x45,0x1}, null);
            sunmiPrinterService.printTextWithFont(ticketInfo.getStadium_name() + "\n", null, 40, null);
...
//                    null, 12, null);
            if (paper == 1) {
                sunmiPrinterService.printText("------------门票信息------------\n", null);
            } else {
                sunmiPrinterService.printText("--------------------门票信息--------------------\n",
                        null);
            }
            ...
            String txts[] = new String[]{"门票名称", ticketInfo.getTicket_name()};
            int width[] = new int[]{1, 1};
            int align[] = new int[]{0, 2};
            sunmiPrinterService.printColumnsString(txts, width, align, null);
            txts[0] = "门票票号";
            txts[1] = ticketInfo.getTicket_code();
            sunmiPrinterService.printColumnsString(txts, width, align, null);
            txts[0] = "金额";
            txts[1] = "¥" + ticketInfo.getPay_price();
            sunmiPrinterService.printColumnsString(txts, width, align, null);
//            try {
...
//            }
            if (paper == 1) {
                sunmiPrinterService.printText("------------客户信息------------\n", null);
            } else {
                sunmiPrinterService.printText("--------------------客户信息--------------------\n",
                        null);
            }
            txts[0] = "购票人名称";
            txts[1] = ticketInfo.getName();
            sunmiPrinterService.printColumnsString(txts, width, align, null);
            txts[0] = "购票人手机";
            txts[1] = ticketInfo.getPhone();
            sunmiPrinterService.printColumnsString(txts, width, align, null);
            txts[0] = "购票卡号";
            txts[1] = ticketInfo.getCard_num();
            sunmiPrinterService.printColumnsString(txts, width, align, null);
            if (paper == 1) {
                sunmiPrinterService.printText("---------------状态-------------\n", null);
            } else {
                sunmiPrinterService.printText("---------------------状态-----------------------\n",
                        null);
            }
//            txts[0] = "状态";
//            txts[1] = ticketInfo.getStatus_str();
//            sunmiPrinterService.printColumnsString(txts, width, align, null);
            sunmiPrinterService.printText("状态                  ", null);
            sunmiPrinterService.printTextWithFont(ticketInfo.getStatus_str() + "\n", null, 40, null);
//            txts[0] = "检票时间";
//            txts[1] = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date());
//            sunmiPrinterService.printColumnsString(txts, width, align, null);
            sunmiPrinterService.printText("检票时间     " + new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date()), null);
            sunmiPrinterService.autoOutPaper(null);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
